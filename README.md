...
LSP/completion is native: [mason.nvim](https://github.com/mason-org/mason.nvim) + [nvim-lspconfig](https://github.com/neovim/nvim-lspconfig) install and enable `ts_ls`/`omnisharp`/`jsonls` (1:1 replacing coc.nvim's `coc-tsserver`/`coc-omnisharp`/`coc-json`), with [blink.cmp](https://github.com/saghen/blink.cmp) for completion. `gd`/`gy`/`gi`/`gr`/`<leader>rn`/`<leader>ca`/`<leader>f`/`[g`/`]g` all now come from `vim.lsp.buf.*`/`vim.diagnostic.*` rather than coc; `K` still checks for a hover-capable client first and falls back to devdocs.vim's buffer-local mapping otherwise, same logic as before. ALE is unchanged, still covering filetypes the LSP servers here don't.
