...
"
" Neovim (see nvim/) is the primary editor now; this file is deliberately
" a zero-dependency fallback -- no plugin manager, no plugins, nothing to
" install. It only has to work on a fresh or minimal box where `vim` is
" whatever the system ships. Everything that needed a plugin (airline,
" solarized8, tmuxline, ALE, coc.nvim, gitgutter, fugitive, rhubarb,
" devdocs, vim-javascript) lives in the Neovim config instead; git
" history has the previous version if any of it is ever wanted back.

set nocompatible

" Colours {{{
" Plain 'syntax enable' with a dark background -- the Solarized colorscheme
" was a plugin, so no colorscheme is set here on purpose.
syntax enable
